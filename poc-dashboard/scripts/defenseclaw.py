...
Deploys a single Pod with two containers:
  ...
  2. DefenseClaw 0.8.10 — security gateway + guardrail (installed via its signed
     installer, gateway API on port 18790)
...
# DefenseClaw 0.8.10 — installed via its official signed installer (needs only
# Python 3.10-3.13 + uv). This replaces the 0.2.x Go-binary tarball + wheel:
# 0.8.x is a different architecture (v8 observability config, new CLI, a managed
# `defenseclaw-gateway` daemon).
DEFENSECLAW_VERSION = "0.8.10"
DEFENSECLAW_INSTALL_URL = (
    f"https://raw.githubusercontent.com/cisco-ai-defense/defenseclaw/"
    f"{DEFENSECLAW_VERSION}/scripts/install.sh"
)
# DefenseClaw 0.8.10's own installer pins OpenClaw to this build; keep the
# OpenClaw container on the same version to avoid gateway/protocol drift.
OPENCLAW_VERSION = "2026.3.24"

# OpenClaw WebChat gateway port + DefenseClaw guardrail API port (shared pod localhost).
OPENCLAW_PORT = 18789
GATEWAY_API_PORT = 18790
...
def deploy_environment():
    ...
    # DefenseClaw 0.8.10 uses config_version: 8, generated by its own CLI
    # (`defenseclaw init` / `setup guardrail` / `setup splunk`) rather than a
    # hand-written flat config — so only OpenClaw's config is shipped here.
    ...
    openclaw_container = client.V1Container(
        ...
        args=[textwrap.dedent("""\
            set -e
            echo "[openclaw] Installing OpenClaw (pinned to the DefenseClaw 0.8.10-matched build)..."
            npm install -g openclaw@2026.3.24 2>&1 | tail -5
    ...
    # ── Container 2: DefenseClaw 0.8.10 ──────────────────────────────────
    # Installs via the official signed installer (Python + uv), attaches to the
    # sibling OpenClaw gateway on shared-pod localhost, enables the guardrail in
    # action mode, and forwards audit events to the existing PoC Splunk HEC.
    #
    # NOTE (needs live validation on the cluster): DefenseClaw 0.8.x is designed
    # to run as a managed daemon on a host with systemd, not as a foreground
    # container process. The install/CLI flow below follows the 0.8.10 docs, but
    # the exact connector-attach keys, guardrail/API ports, and the way the
    # `defenseclaw-gateway` daemon is kept alive in a container may need tuning
    # against `kubectl logs`. It is intentionally tolerant (|| true) so a single
    # failed step doesn't crash-loop the whole pod before you can inspect it.
    defenseclaw_container = client.V1Container(
        ...
        args=[textwrap.dedent("""\
            set -e
            export HOME=/root
            export PATH="/root/.local/bin:$HOME/.defenseclaw/.venv/bin:$PATH"

            echo "[defenseclaw] Installing prerequisites (curl, ca-certificates, uv)..."
            apt-get update -qq && apt-get install -y -qq curl ca-certificates >/dev/null 2>&1
            curl -LsSf https://astral.sh/uv/install.sh | sh >/dev/null 2>&1 || true

            echo "[defenseclaw] Installing DefenseClaw {version} (no bundled OpenClaw; using the sibling container)..."
            curl -LsSf "{install_url}" | VERSION="{version}" bash -s -- --no-openclaw --yes

            echo "[defenseclaw] Initializing (guardrail enabled, OpenClaw connector)..."
            defenseclaw init --enable-guardrail --connector openclaw --non-interactive --yes || true

            # Guardrail in ACTION mode so flagged prompts/responses are actually blocked in the demo.
            defenseclaw setup guardrail --mode action || true

            # Forward audit events to the EXISTING PoC Splunk (not the bundled local Splunk).
            # The HEC token is read from DEFENSECLAW_SPLUNK_HEC_TOKEN in the environment.
            if [ -n "$DEFENSECLAW_SPLUNK_HEC_TOKEN" ]; then
                echo "[defenseclaw] Configuring Splunk HEC export -> {hec_url}..."
                defenseclaw setup splunk --enterprise \\
                    --hec-endpoint "{hec_url}/services/collector/event" \\
                    --hec-token "$DEFENSECLAW_SPLUNK_HEC_TOKEN" \\
                    --index defenseclaw \\
                    --non-interactive --skip-test || true
            fi

            # Wait for the sibling OpenClaw gateway (shares pod localhost).
            echo "[defenseclaw] Waiting for OpenClaw gateway on localhost:{openclaw_port}..."
            ...
            echo "[defenseclaw] Starting the DefenseClaw gateway daemon..."
            defenseclaw-gateway start || true

            # Keep the container in the foreground. The gateway runs as a managed
            # daemon, so tail its state via the CLI status loop instead of exec'ing
            # a foreground process (there is no documented foreground gateway mode).
            echo "[defenseclaw] Gateway started; holding container open."
            while true; do
                defenseclaw status >/dev/null 2>&1 || true
                sleep 30
            done
        """.format(
            version=DEFENSECLAW_VERSION,
            install_url=DEFENSECLAW_INSTALL_URL,
            hec_url=SPLUNK_HEC_URL,
            openclaw_port=OPENCLAW_PORT,
        ))],
        ports=[
            client.V1ContainerPort(container_port=GATEWAY_API_PORT, name="api"),
        ],
        env=[
            api_key_env,
            token_env,
            # DefenseClaw 0.8.x reads the Splunk HEC token from this env var and
            # writes only a token_env reference into its config (no secret at rest).
            client.V1EnvVar(name="DEFENSECLAW_SPLUNK_HEC_TOKEN", value=HEC_TOKEN),
            # Token used to authenticate to the sibling OpenClaw gateway.
            client.V1EnvVar(name="OPENCLAW_GATEWAY_TOKEN", value=shared_token),
        ],
        volume_mounts=[
            client.V1VolumeMount(name="openclaw-home", mount_path="/openclaw-home",
                                 read_only=True),
